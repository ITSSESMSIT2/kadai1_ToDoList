...
const date = dateNum.padStart(2, "0");
const defaultDay = `${year}-${month}-${date}`;

const setDefaultDay = document.querySelector("#taskLimit");
setDefaultDay.value = defaultDay;
...
form.addEventListener("submit", function (e) {
  ...
  e.preventDefault();
  // ここで入力欄の空欄を確認。→配列に空が格納されるのを防止する
  const name = taskName.value.trim();
  if (name === "" || name === null) {
    alert("タスク名を入力してください");
    // ここでreturnを用いることで、入力値を保持したままにできる。
    return;
  } else {
    taskId++;
    const task = {
...
ul.addEventListener("click", function (ev) {
  ...
  if (ev.target.nodeName === "BUTTON") {
    ...
    const deliteTargetIndex = taskList.findIndex(checkId);
    if (deliteTargetIndex !== -1) {
...
ul.addEventListener("change", function (eve) {
  if (eve.target.nodeName === "SELECT") {
    ...
    const changeTargetIndex = taskList.findIndex(checkId);
